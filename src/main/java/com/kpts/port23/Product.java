...
public class Product {
	...
	private InterpolatedNodalCurve CURVE_REPO = InterpolatedNodalCurve.of(METADATA_REPO, DoubleArray.of(0.1, 2.0, 10.0),
	...
	private InterpolatedNodalCurve CURVE_ISSUER = InterpolatedNodalCurve.of(METADATA_ISSUER,
			DoubleArray.of(0.2, 9.0, 15.0), DoubleArray.of(0.03, 0.05, 0.13), INTERPOLATOR);
	...
	public Product(String sECURITY_ID, String iSSUER_ID, long qUANTITY, double nOTIONAL, double fIXED_RATE,
			String sTART_DATE, String eND_DATE, String settlementDate, double cleanPrice, String valDate,
			String rowNum) {
		this.rowNum = rowNum;
		...
		UPFRONT_PAYMENT = Payment.of(CurrencyAmount.of(EUR, -QUANTITY * NOTIONAL * DIRTY_PRICE), SETTLEMENT);
		CalculateTrade();
	...
	private LegalEntityDiscountingProvider createRatesProvider(LocalDate valuationDate) {
		...
		LegalEntityDiscountingProvider provider = ImmutableLegalEntityDiscountingProvider.builder()
				...
				.repoCurveSecurityGroups(ImmutableMap.of(SECURITY_ID, GROUP_REPO)).valuationDate(valuationDate).build();
		return provider;
	}

	private void CalculateTrade() {
		TRADE = ResolvedFixedCouponBondTrade.builder().product(PRODUCT).quantity(QUANTITY)
	...
	private void CalculateProduct() {
		...
		try {
			PRODUCT = FixedCouponBond.builder().securityId(SECURITY_ID).dayCount(DAY_COUNT).fixedRate(FIXED_RATE)
	...
	public StringBuilder calculatePresentValue() {
	...
	public String toString() {
		return "Product [REF_DATA=" + REF_DATA + ", SECURITY_ID=" + SECURITY_ID + ", ISSUER_ID=" + ISSUER_ID
				+ ", QUANTITY=" + QUANTITY + ", YIELD_CONVENTION=" + YIELD_CONVENTION + ", NOTIONAL=" + NOTIONAL
				+ ", FIXED_RATE=" + FIXED_RATE + ", EUR_CALENDAR=" + EUR_CALENDAR + ", DATE_OFFSET=" + DATE_OFFSET
				+ ", DAY_COUNT=" + DAY_COUNT + ", START_DATE=" + START_DATE + ", END_DATE=" + END_DATE + ", VAL_DATE="
				+ VAL_DATE + ", SETTLEMENT=" + SETTLEMENT + ", BUSINESS_ADJUST=" + BUSINESS_ADJUST
				+ ", PERIOD_SCHEDULE=" + PERIOD_SCHEDULE + ", EX_COUPON=" + EX_COUPON + ", CLEAN_PRICE=" + CLEAN_PRICE
				+ ", TRADE_PRICER=" + TRADE_PRICER + ", PRODUCT_PRICER=" + PRODUCT_PRICER + ", PRICER_NOMINAL="
				+ PRICER_NOMINAL + ", PRODUCT=" + PRODUCT + ", DIRTY_PRICE=" + DIRTY_PRICE + ", UPFRONT_PAYMENT="
				+ UPFRONT_PAYMENT + ", TRADE=" + TRADE + ", PROVIDER=" + PROVIDER + ", INTERPOLATOR=" + INTERPOLATOR
				+ ", NAME_REPO=" + NAME_REPO + ", METADATA_REPO=" + METADATA_REPO + ", CURVE_REPO=" + CURVE_REPO
				+ ", GROUP_REPO=" + GROUP_REPO + ", GROUP_ISSUER=" + GROUP_ISSUER + ", NAME_ISSUER=" + NAME_ISSUER
				+ ", METADATA_ISSUER=" + METADATA_ISSUER + ", CURVE_ISSUER=" + CURVE_ISSUER + ", rowNum=" + rowNum
				+ ", isprinted=" + isprinted + "]";
	}
